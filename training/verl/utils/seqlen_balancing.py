...
def rearrange_mm_micro_batches(batch, multi_modal_inputs, max_token_len, dp_group=None, num_batches_divided_by=None, same_micro_num_in_dp=True, min_num_micro_batch=None):
    """
    Split a batch into micro-batches by total token count, with optional DP sync and padding.
    ...
    for partition in micro_bsz_idx:
        ...
        for idx in partition:
            curr_micro_batch.append(batch[idx : idx + 1])
            curr_mm_micro_batch.append(multi_modal_inputs['multi_modal_inputs'][idx])
        curr_micro_batch = torch.cat(curr_micro_batch)

        micro_batches.append(curr_micro_batch)
        multi_modal_micro_batches.append({'multi_modal_inputs': curr_mm_micro_batch})

    print(f"rearranging micro batches, {batch['attention_mask'].shape=} {seq_len_effective=}, {total_seqlen=}, {num_micro_batches=} {micro_bsz_idx=} {len(micro_batches)=} {micro_batches[0].shape=}")

    return micro_batches, multi_modal_micro_batches, micro_bsz_idx

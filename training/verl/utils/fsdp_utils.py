...
# Fix mixed torch.Tensor and DTensor error
def apply_fsdp2(model, fsdp_kwargs, config):
    """model: AutoModelForCausalLM"""
    assert CPUOffloadPolicy is not None, "PyTorch version >= 2.4 is required for using fully_shard API (FSDP2)"
    ...
    if isinstance(model, Glm4vForConditionalGeneration):
        from transformers.models.glm4v.modeling_glm4v import Glm4vVisionEmbeddings
        skip_prefixes = {name for name, mod in model.named_modules() if isinstance(mod, Glm4vVisionEmbeddings)}
    else:
        skip_prefixes = set()
    for name, module in model.named_modules():

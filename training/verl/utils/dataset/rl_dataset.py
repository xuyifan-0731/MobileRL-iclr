...
class AgenticDataset(Dataset):
    ...
    def __getitem__(self, item):
        #print("in getitem", self.indices[item])
        return {
            "index": torch.tensor(self.indices[item]),
            "name": self.names[item],
            "data_source": self.names[item],
        }

class AgenticDatasetOrigin(Dataset):
    def __init__(
        self,
        name: str,
        index_start: int,
    ...
    def _read_files_and_tokenize(self):
        ...
        if self.filter_overlong_prompts:
            ...
            if self.task_type == 'default':
                ...
                self.dataframe = self.dataframe.filter(lambda doc: len(

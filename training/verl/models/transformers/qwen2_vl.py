...
try:
    ...
    elif is_transformers_version_in_range(min_version="4.53.0", max_version="4.54.9"):
        from transformers.modeling_flash_attention_utils import _lazy_imports
        flash_attn_func, flash_attn_varlen_func, pad_input, unpad_input, is_fa3 = _lazy_imports("flash_attention_2")
    else:
...
def ulysses_flash_attn_forward(
    ...
    **kwargs,
    ...
    if config is not None:
        assert config._attn_implementation == "flash_attention_2", "config._attn_implementation must be 'flash_attention_2'"

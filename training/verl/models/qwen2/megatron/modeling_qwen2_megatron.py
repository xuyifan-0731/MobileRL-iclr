...
class ParallelQwen2ModelRmPadPP(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, megatron_config: ModelParallelConfig, pre_process, post_process, pp_layers):
        super().__init__()
        self.config: TransformerConfig = convert_config(config, megatron_config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        ...
        if not pp_layers:
            pp_layers = [config.num_hidden_layers // pp_size] * pp_size
        assert pp_size == sum(pp_layers), f"{pp_size=} != {sum(pp_layers)=}"
        self.num_layer_per_pp = pp_layers[pp_rank]
...
class ParallelQwen2ForCausalLMRmPadPP(nn.Module):
    def __init__(
        ...
        self.config: TransformerConfig = convert_config(config, megatron_config)
        self.megatron_config = megatron_config
        self.model = ParallelQwen2ModelRmPadPP(config, megatron_config=megatron_config, pre_process=pre_process, post_process=post_process, pp_layers=pp_layers)

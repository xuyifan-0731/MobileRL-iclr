...
def compute_policy_loss(
    ...
    print(f'in_compute_policy_loss_{response_mask.sum()=}')

    negative_approx_kl = log_prob - old_log_probs
    ...
    is_ratio = torch.exp(old_log_probs - behavior_log_probs).detach()
    is_ratio = torch.min(is_ratio, torch.ones_like(is_ratio) * 2)
    pg_losses = pg_losses * is_ratio

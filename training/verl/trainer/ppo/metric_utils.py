...
def compute_data_metrics(batch, use_critic=True, tokenizer=None, prefix="rl"):
    ...
    def calc_batch_rl_metrics(batch):
        sequence_score = batch.batch['token_level_scores'].sum(-1)
        ...
        for uuid, score in zip(batch.non_tensor_batch['uuid'], sequence_score):
            uuid_scores[uuid].append(score.item())
        ...
        for resp, mask in zip(batch.batch['responses'], response_mask):
            ...
            turn_overlong_ratios.append(resp[resp_len - 1] != tokenizer.pad_token_id)
        ...
        overlong_ratio = sum(a or b for a, b in zip(length_overlong_ratios, turn_overlong_ratios)) / len(
            length_overlong_ratios)

        ret = {
            "average_reward": torch.mean(binary_sequence_score).detach().item(),
            "traj_reward": sum([max(scores) for scores in uuid_scores.values()]) / len(uuid_scores),
            ...
            "average_turns": sum([len(scores) for scores in uuid_scores.values()]) / len(uuid_scores),
            ...
            "average_repeat": sum([len(scores) for scores in uid_scores.values()]) / len(uid_scores),
            "length_overlong_ratio": length_overlong_ratio,
            "turn_overlong_ratio": turn_overlong_ratio,
            "overlong_ratio": overlong_ratio,
        }

        for key in batch.meta_info["obs_keys"]:
            if key in batch.batch:
                values = batch.batch[key].float()
                if values.isnan().all():
                    continue
                ret[f"{key}"] = torch.mean(values[~values.isnan()]).detach().item()

        return ret

    rl_metrics = {f"{prefix}/{k}": v for k, v in calc_batch_rl_metrics(batch).items()}

    unique_sources = np.unique(batch.non_tensor_batch['data_source'])
    for source in unique_sources:
        source_indices = np.where(batch.non_tensor_batch['data_source'] == source)[0]
        source_batch = batch[source_indices]
        rl_metrics.update({f"{prefix}/{source}/{k}": v for k, v in calc_batch_rl_metrics(source_batch).items()})

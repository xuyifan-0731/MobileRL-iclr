...
from verl.utils.flops_counter import FlopsCounter
from verl.utils.checkpoint.fsdp_checkpoint_manager import FSDPCheckpointManager
from verl.workers.abstract import AbstractActorWorker, AbstractRolloutWorker
...
def create_device_mesh(world_size, fsdp_size):
    ...
    device_mesh = init_device_mesh(device_name, mesh_shape=(dp_size, fsdp_size), mesh_dim_names=("dp", "fsdp"))
    return device_mesh
...
class ActorRolloutRefWorker(Worker, AbstractActorWorker, AbstractRolloutWorker):
    """
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        # normalize config
        if self._is_actor:
            # multiplying n is moved to main_ppo.py
    ...
    def _build_model_optimizer(
        ...
        self.processor = hf_processor(local_path, trust_remote_code=trust_remote_code, max_pixels=self.config.rollout.max_pixels, min_pixels=self.config.rollout.min_pixels, use_fast=True)
        ...
        with init_context(), warnings.catch_warnings():
            ...
            elif actor_model_config.model_type == "glm4v":
                from transformers import Glm4vForConditionalGeneration
                actor_module_class = Glm4vForConditionalGeneration
            else:
        ...
        print(f'{role} module Mixed precision: {mixed_precision}')
        ...
        if role == "actor" and optim_config is not None:
            print(f"optim_config: {optim_config}, override_model_config: {override_model_config}")
            from verl.utils.torch_functional import get_constant_schedule_with_warmup, get_cosine_schedule_with_warmup
            if optim_config.get('freeze_vit', False):
                ...
                for name, param in actor_module_fsdp.named_parameters():
                    print(f'after change: param name: {name}, requires_grad: {param.requires_grad}')
    ...
    def _build_rollout(self, model_path, trust_remote_code=False):
        from torch.distributed.device_mesh import init_device_mesh
        ...
        rollout_device_mesh = init_device_mesh(device_name, mesh_shape=(dp, infer_tp), mesh_dim_names=["dp", "tp"])
    ...
    def init_model(self):
        ...
        if self._is_rollout:
            ...
            local_path = copy_to_local(self.config.model.path, use_shm=use_shm)
            self.rollout, self.rollout_sharding_manager = self._build_rollout(model_path=local_path)
        elif self._is_actor:
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    @GPUMemoryLogger("sync_params", logger=logger)
    def sync_params(self, no_rnd=False):
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)

        self.rollout_sharding_manager.sync_params(no_rnd)
    ...
    def get_async_seqs(self, buffer_out, minimum, multiple):
        ...
        for other, result in zip(others, results):
            other.non_tensor_batch["uuid"] = str(uuid.uuid4())
            flattened_others.extend([copy.deepcopy(other) for _ in range(len(result))])
        other = collate_fn(flattened_others)
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        self.rollout_sharding_manager.offload()
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if self.config.get("zero_init_value_head", False):
                value_head: torch.nn.Linear = critic_module.score
